...
#include "../STM32CubeIDE/Application/User/sliding_mode_observer.h"
...
/**
 * @brief  Sliding Mode Observer instance (replaces STO_PLL_M1).
 *
 * Scale factors for normalised-to-physical conversion:
 *   Voltage: SDK uses s16V where 32767 = (Vref/sqrt(3)) / VBUS_PARTITIONING.
 *            From parameters_conversion.h: MAX_VOLTAGE = (3.3/1.732)/0.052 ≈ 36.6 V
 *            So  1 s16V unit ≈ 36.6/32767 ≈ 1.116e-3 V
 *   Current: SDK uses s16A where 32767 = Vref/(2*Rshunt*Gain) ≈ 22.6 A
 *            So  1 s16A unit ≈ 22.6/32767 ≈ 6.9e-4 A
 *
 * The SMO is initialised with Rs/Ls in physical units; inputs v_α/β and
 * i_α/β are passed as normalised SDK digits (int16_t cast to float).
 * This is valid because the motor model equations are linear in V and I:
 * the ratio b = (1-A)/Rs already encodes the unit scaling.
 *
 * IMPORTANT: smo_min_emf must be re-tuned in the same normalised units.
 * With max current ≈ 22.6 A and Ke=0.6 V/kRPM:
 *   BEMF at 900 RPM ≈ 0.6*0.9*√2 ≈ 0.76 V → 0.76/1.116e-3 ≈ 681 digits
 * Set min_emf conservatively at 300 digits (normalised squared threshold
 * handled in smo_update() as mag2 > thr2 where thr = min_emf in digits).
 */
static smo_t SMO_M1;

/* Sampling period = 1/TF_REGULATION_RATE = 1/(PWM_FREQ/REG_EXEC_RATE)
 * = REGULATION_EXECUTION_RATE / PWM_FREQUENCY = 3/16000 = 187.5 µs     */
#define SMO_TS_S    (REGULATION_EXECUTION_RATE / (float)PWM_FREQUENCY)

/* Physical motor parameters from pmsm_motor_parameters.h and
 * power_stage_parameters.h                                               */
#define SMO_RS_OHM  (float)RS          /* 0.06 Ω */
#define SMO_LS_H    (float)LS          /* 10 µH  */
#define SMO_POLE_PAIRS (float)POLE_PAIR_NUM  /* 7  */
